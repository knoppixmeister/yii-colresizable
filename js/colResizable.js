(function($) {
	...
	var init = function(tb, options) {	
		...
		t.cs = parseInt($.browser.msie ? tb.cellSpacing || tb.currentStyle.borderSpacing :t.css('border-spacing'))||2;	//table cellspacing (not even jQuery is fully cross-browser)
		t.b  = parseInt($.browser.msie ? tb.border || tb.currentStyle.borderLeftWidth :t.css('border-left-width'))||1;	//outer border width (again cross-browser isues)
	...
	*/
	var syncCols = function(table, index, isOver) {
		...
		/*
		var column	=	table.c[index];
		var column2	=	table.c[index+1];
		var width	=	column.w+inc;
		var width2	=	column2.w-inc;		//their new width is obtained

		column.width(width+PX);
		column2.width(width2+PX);
		table.cg.eq(index).width(width+PX);
		table.cg.eq(index+1).width(width2+PX);
		*/

		for(var i=0; i<=index; i+=2) {
			var column	= table.c[i];
			var column2	= table.c[i+1];
			var width	= column.w+inc;
			var width2	= column2.w-inc;
	...
	var onGripDragOver = function(e) {
		$(document).unbind('mousemove.'+SIGNATURE)
					.unbind('mouseup.'+SIGNATURE);
	...
	var onResize = function() {
		for(table in tables) {
			...
			if(table.w != table.width()) {							//if the the table's width has changed
				table.w = t.width();							//its new value is kept
				for(i=0; i<t.ln; i++) {
				...
				for(i=0; i<t.ln; i++) {
					table.c[i].css("width", Math.round(1000*table.c[i].w/mw)/10 + "%").l = true;
				}
